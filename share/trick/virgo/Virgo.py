...
And for your viewing pleasure, here is an "ASCII art representation
of the Virgo Maiden" as generated by AI in July 2025 (prepare yourself)
...
class VirgoActor(vtk.vtkActor):
    ...
    def __init__(self, mesh, init_pyr=None, times=None, positions=None, rotations=None, name='No Name', fontsize=12):
        """
        mesh: 3D representation of the actor in the scene, which is either:
          1. a path to an object file to load like .obj, .stl, or other formats
             understood by vtkOBJReader
          OR
          2. a path to an object file to load like .obj, .stl, or other formats
        init_pyr: X-Y-Z (Pitch-Yaw-Roll) rotation to apply to the model on init
        TODO: Probably need init_pos offset
        times: Sorted ascending list of double representing all time steps for all data
           associated with this actor
        positions: List of tuples of (x, y, z) positions of this actor, one per entry
           in times list. Length of positions must == length of times
        rotations : List of 3X3 numpy array of doubles (rotation matrix), one per entry
           in times list.  Length of rotations must == length of times
        name: Name given to this actor
        fontsize: Default fontsize for text associated with this actor
        """
...
class VirgoDataPlaybackControlCenter:
    ...
    def add_axes_to_actors(self):
        ...
        for name, actor in self.actors.items():
            ...
            # Set axes properties
            axes.SetTotalLength(length, length, length)  # Size of axes (x, y, z lengths)
